...
`docs/performance/README.md` を最新の計測結果で更新します。
...
1. **計測結果の確認**
   - `perf_results/` ディレクトリの最新ファイルを確認
   - `benchmark_results/` ディレクトリの最新ファイルを確認
...
3. **前回計測との比較分析**
   - `docs/performance/README.md` の現在の値と新しい計測値を比較
...
4. **ドキュメント更新**
   - `docs/performance/README.md` のホットスポット一覧を更新
   - 計測日を更新
   - 変更履歴に追記（前回比較の分析結果を含める）

## 実行

最新の計測結果ファイルを読み込み、`docs/performance/README.md` を更新してください。
