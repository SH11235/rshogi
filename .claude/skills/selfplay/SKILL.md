...
- 標準 bounds は用途別 2 種 (α=β=0.05):
  ...
  - 負値は現行バイナリならそのまま通る。`error: unexpected argument '-1' found` が出る古い
    tournament (負値対応前) では `--sprt-nelo0=-10` の `=` 形式で渡す。
...
(負値対応前の古い tournament で `unexpected argument '-1'` になる場合は `--sprt-nelo0=-10`)。
