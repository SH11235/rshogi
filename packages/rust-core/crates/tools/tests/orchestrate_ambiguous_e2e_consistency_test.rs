...
#[test]
fn orchestrate_ambiguous_e2e_external_normalize_consistency() {
    // Same E2E but force external normalize with tiny chunks and fan-in 2
    let tmp = TempDir::new().unwrap();
    let pass1 = tmp.path().join("p1.jsonl");
    // 3 lines ensure chunk_lines=1 => 3 chunks, triggers multi-pass with fan-in 2
    write_pass1_jsonl(&pass1, 3);
    ...
    // Final outputs exist and manifest written_lines matches file
    assert!(final_out.exists());
    let final_manifest = tmp
        .path()
        .join(format!("{}.manifest.json", final_out.file_stem().unwrap().to_string_lossy()));
    ...
    let final_lines = s.lines().count() as u64;
    let mani_txt = fs::read_to_string(&final_manifest).expect("read final manifest");
    let v: Value = serde_json::from_str(&mani_txt).expect("parse final manifest");
    let w = v
        .get("aggregated")
        .and_then(|a| a.get("written_lines"))
        .and_then(|x| x.as_u64())
