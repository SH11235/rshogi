...
/// Strict YO MATERIAL Lv.3 evaluator adapter implementing `Evaluator`.
struct YoMaterialEvaluator;

impl Evaluator for YoMaterialEvaluator {
    fn evaluate(&self, pos: &Position) -> i32 {
        evaluate_yo_material_lv3(pos)
    }
}

/// Run ClassicAB search with strict YO MATERIAL Lv.3 evaluator against a single SFEN.
///
/// `classicab_itertrace` の Lv.3 版であり、評価関数として
/// `evaluate_yo_material_lv3` をそのまま利用する。
#[derive(Parser, Debug)]
#[command(
    name = "classicab_lv3_itertrace",
    about = "Run ClassicAB search with strict YO MATERIAL Lv.3 evaluator for a single SFEN",
...
fn main() -> Result<()> {
    ...
    let evaluator = Arc::new(YoMaterialEvaluator);
    ...
    for run_idx in 0..args.repeat {
        ...
        println!(
            "starting_search_lv3 depth={} hash_mb={} nmp={} iid={} probcut={} razor={} sbp={}",
            args.depth,
            args.hash_mb,
            !args.disable_nmp,
            !args.disable_iid,
        ...
        println!(
            "search_completed_lv3 nodes={} seldepth={} score={} reason={} best_move={}",
