...
///
/// Additionally, we handle cases where USI notation includes a promotion flag ('+')
/// but the move cannot actually promote (e.g., "2b8h+" for a pawn moving backwards).
fn moves_equal(m1: Move, m2: Move) -> bool {
    ...
    // For promotion: either they match exactly, or m1 tries to promote but m2 doesn't
    // (This handles cases where USI notation includes '+' but promotion is not possible)
    (m1.is_promote() == m2.is_promote() || (m1.is_promote() && !m2.is_promote()))
}
