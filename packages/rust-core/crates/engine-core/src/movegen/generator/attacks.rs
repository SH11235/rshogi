...
/// Get all attackers of a square from the given color (excluding king, pawn drop, and lance drop checks)
pub(super) fn attackers_to_except_king_pawn_lance(
    gen: &MoveGenImpl,
    sq: Square,
    color: Color,
) -> Bitboard {
...
/// Get all attackers to a square (standard occupancy)
pub(super) fn attackers_to(gen: &MoveGenImpl, sq: Square, color: Color) -> Bitboard {
    let occupancy = gen.pos.board.all_bb;
    attackers_to_with_occupancy(gen, sq, color, occupancy)
}
