...
mod tests {
    ...
    fn test_debug_accumulator_values() {
        ...
        let file = match File::open(path) {
            Ok(f) => f,
            Err(_) => {
                eprintln!("Skipping test: nn.bin not found at {}", path);
                return;
            }
        };
        let mut reader = BufReader::new(file);

        // FeatureTransformerを直接読み込み
        // バージョンとハッシュとアーキテクチャ文字列を読み飛ばす
        let mut buf4 = [0u8; 4];
        ...
        let ft = FeatureTransformer::read(&mut reader).unwrap();

        // バイアス値の確認
        let bias_sum: i64 = ft.biases.0.iter().map(|&v| v as i64).sum();
        let bias_min = *ft.biases.0.iter().min().unwrap();
        let bias_max = *ft.biases.0.iter().max().unwrap();
        eprintln!("Bias: sum={}, min={}, max={}", bias_sum, bias_min, bias_max);
        eprintln!("Bias first 10: {:?}", &ft.biases.0[..10]);
        ...
        // 特徴量の数を確認
        let active_b = HalfKPFeatureSet::collect_active_indices(&pos, Color::Black);
        let active_w = HalfKPFeatureSet::collect_active_indices(&pos, Color::White);
        eprintln!("Active features: Black={}, White={}", active_b.len(), active_w.len());

        // 両視点のインデックスを比較
        let mut indices_b: Vec<_> = active_b.iter().copied().collect();
        let mut indices_w: Vec<_> = active_w.iter().copied().collect();
        indices_b.sort();
        indices_w.sort();
        let same_count = indices_b.iter().zip(&indices_w).filter(|(a, b)| a == b).count();
        eprintln!("Same indices (sorted): {} / {}", same_count, indices_b.len());
        eprintln!("Black sorted first 5: {:?}", &indices_b[..5.min(indices_b.len())]);
        eprintln!("White sorted first 5: {:?}", &indices_w[..5.min(indices_w.len())]);
        eprintln!("Black sorted last 5: {:?}", &indices_b[indices_b.len().saturating_sub(5)..]);
        eprintln!("White sorted last 5: {:?}", &indices_w[indices_w.len().saturating_sub(5)..]);

        // アキュムレーター値の確認
        for perspective in [Color::Black, Color::White] {
            let values = acc.get(perspective as usize, 0);
            let sum: i64 = values.iter().map(|&v| v as i64).sum();
            let min = *values.iter().min().unwrap();
            let max = *values.iter().max().unwrap();
            eprintln!("{:?}: sum={}, min={}, max={}", perspective, sum, min, max);
            eprintln!("  First 10: {:?}", &values[..10]);
        }

        // 重みのサンプル確認
        // 最初のアクティブな特徴量の重みを確認
        let first_idx = active_b.iter().next().copied().unwrap_or(0);
        let offset = first_idx * TRANSFORMED_FEATURE_DIMENSIONS;
        if offset + 10 <= ft.weights.len() {
            let weight_slice = &ft.weights[offset..offset + 10];
            eprintln!("Weight for first active feature (idx={}): {:?}", first_idx, weight_slice);
        }

        // Transform出力を確認
        let mut output = [0u8; TRANSFORMED_FEATURE_DIMENSIONS * 2];
        ft.transform(&acc, Color::Black, &mut output);
        let nonzero_count = output.iter().filter(|&&x| x != 0).count();
        let sum: u64 = output.iter().map(|&x| x as u64).sum();
        eprintln!("Transform output: {} nonzero / {}, sum={}", nonzero_count, output.len(), sum);
        eprintln!("Transform first 10: {:?}", &output[..10]);
        eprintln!("Transform at 256 (White start): {:?}", &output[256..266]);

        // 完全なネットワーク評価をテスト
        use crate::nnue::Network;
        let path =
            "/home/sh11235/development/shogi/packages/rust-core/memo/nnue-pytorch/eval/nn.bin";
        if let Ok(network) = Network::load(path) {
            let score = network.evaluate(&pos, &acc);
            eprintln!("Network.evaluate(): {}", score.raw());
        }
    }
}
