    ...
    pub fn evaluate(&self, pos: &Position, acc: &Accumulator) -> Value {
        match self {
            ...
            Self::HalfKPDynamic(_) => {
                unreachable!(
                    "BUG: wrong accumulator type - HalfKPDynamic requires \
                     AccumulatorHalfKPDynamic. Use evaluate_halfkp_dynamic() instead."
                )
            }
            Self::HalfKA512(_) => {
                unreachable!(
