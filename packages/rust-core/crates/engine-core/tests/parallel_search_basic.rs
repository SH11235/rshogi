...
use engine_core::{
    ...
    search::{
        parallel::{ParallelSearcher, SearchThread, SharedSearchState, StopController},
...
fn test_parallel_searcher_integration() {
    ...
    let mut searcher = ParallelSearcher::new(evaluator, tt, 4, Arc::new(StopController::new()));
    let mut position = Position::startpos();
