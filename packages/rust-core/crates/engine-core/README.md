...
- リードウィンドウ停止時に `Finalize(Planned)` を送出する挙動は既定で有効です。互換性検証などで従来動作に戻したい場合は `SHOGI_LEAD_WINDOW_FINALIZE=off`（`0` / `false` も可）を環境変数に設定してください。CI やテストではこの変数を設定せず（既定 ON のまま）にしておくことで、リードウィンドウ関連のテストが安定します。
