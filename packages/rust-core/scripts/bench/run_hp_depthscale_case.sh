#!/usr/bin/env bash
set -euo pipefail
# moved: scripts/bench/run_hp_depthscale_case.sh

# Run a single HP depth-scale case with SafePruning on/off
# Usage: CUT_AT=5a4b scripts/run_hp_depthscale_case.sh <depth_scale> <on|off> <threads>
...
ON=$([[ "$SAFE" == on ]] && echo true || echo false)
TAG="hpds_${SCALE}_safe_${SAFE}_t${THREADS}"
...
echo "[hp-depthscale] scale=${SCALE} safe=${SAFE} threads=${THREADS} -> $OUT" >&2
