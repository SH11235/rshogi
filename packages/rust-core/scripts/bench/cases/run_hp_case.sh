#!/usr/bin/env bash
set -euo pipefail
# moved: scripts/bench/cases/run_hp_case.sh

# Run a single HP threshold case: value and threads
# Usage: CUT_AT=5a4b scripts/run_hp_case.sh <hp_threshold> <threads>
...
OUT="runs/repro/hp${HP}_threads${THREADS}.log"
mkdir -p runs/repro
echo "[hp-case] HP_THRESHOLD=${HP} threads=${THREADS} -> $OUT" >&2
