#!/usr/bin/env bash
set -euo pipefail
# moved: scripts/bench/run_ab_lmr.sh

# Sweep LMR_K_x100 and compare outcomes.
# Usage: CUT_AT=5a4b scripts/run_ab_lmr.sh
...
LMR_VALUES=( 170 220 260 300 )
THREADS_SET=( 1 8 )

for k in "${LMR_VALUES[@]}"; do
  for th in "${THREADS_SET[@]}"; do
    OUT="runs/repro/lmr${k}_threads${th}.log"
    echo "[ab-lmr] lmr_k=${k} threads=${th} -> $OUT" >&2
    {
      echo usi
...
echo "[ab-lmr] done. grep '^bestmove' runs/repro/lmr*.log"
