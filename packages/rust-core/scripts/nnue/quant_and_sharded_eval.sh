...
# Baseline: compare Classic vs Classic Champion by default to keep ΔNPS comparable
BASE=${BASE:-runs/baselines/current/classic.nnue}

CAND_FP32="$DIR/classic_v1/nn_best.fp32.bin"
while [ ! -f "$CAND_FP32" ]; do sleep 60; done

# 1) Quantize per CAL_LIMITS
CAL_LIMITS=( ${CAL_LIMITS:-120000} )
for LIM in "${CAL_LIMITS[@]}"; do
  SUFFIX="$((LIM/1000))k"
  OUT_Q="$DIR/classic_v1_q_pc_${SUFFIX}"
  cargo run -p tools --release --bin train_nnue -- \
    ...
    --quant-calibration-limit "$LIM" \
    --quant-ft per-tensor --quant-h1 per-channel --quant-h2 per-channel --quant-out per-tensor \
    --final-cp-gain ${FINAL_CP_GAIN:-64} \
    --out "$OUT_Q"

  CAND_INT="$OUT_Q/nn.classic.nnue"

  # 2) ShortTC 800 (sharded)
  S800_OUT="$DIR/gauntlet_shorttc_q_pc_${SUFFIX}_sharded_800"
  mkdir -p "$S800_OUT"
  ...
  if awk "BEGIN{exit !($WIN>=0.55)}"; then
    ...
    S2K_OUT="$DIR/gauntlet_shorttc_q_pc_${SUFFIX}_sharded_2000"
    mkdir -p "$S2K_OUT"
    ...
    L800_OUT="$DIR/gauntlet_longtc_q_pc_${SUFFIX}_sharded_800"
    mkdir -p "$L800_OUT"
