...
mod tests {
    ...
    fn test_move_picker_generates_all_moves() {
        ...
        for &mv in move_list.as_slice() {
            assert!(picker_moves.contains(&mv), "Move {:?} not generated by picker", mv);
        }
    }
    ...
    fn test_move_picker_respects_killers() {
        ...
        for &pos in &killer_positions {
            assert!(pos < 20, "Killer move at position {} is too late", pos);
        }
    }
    ...
    fn test_move_picker_capture_ordering() {
        ...
        if !captures.is_empty() {
            ...
            assert!(
                first_capture_idx < 3,
                "First capture at position {} is too late",
                first_capture_idx
            );
        }
    }
