...
impl SimpleWebRTCPeer {
    ...
    pub async fn create_offer(&self) -> Result<String, JsValue> {
        ...
        wasm_bindgen_futures::JsFuture::from(self.connection.set_local_description(&offer.into()))
            .await?;
        ...
        let json =
            serde_json::to_string(&offer_data).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub async fn handle_offer(&self, offer_str: &str) -> Result<String, JsValue> {
        ...
        let json = String::from_utf8(decoded).map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        wasm_bindgen_futures::JsFuture::from(self.connection.set_local_description(&answer.into()))
            .await?;
        ...
        let json =
            serde_json::to_string(&answer_data).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub async fn create_webrtc_peer(is_host: bool) -> Result<SimpleWebRTCPeer, JsValue> {
    ...
    // Set up connection state change handler
    {
        ...
        let onconnectionstatechange = Closure::wrap(Box::new(move || {
            ...
            dispatch_webrtc_event(
                "connection-state",
                &format!("{:?}", conn_clone.ice_connection_state()),
            );
        }) as Box<dyn FnMut()>);
        connection
