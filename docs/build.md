...
- `scripts/build_*.sh`: ad-hoc build script は canonical atomic feature / preset
  edition を使うのでそのまま動く。旧 feature alias (`layerstack-only` /
  `layerstacks-*` / `nnue-psqt` / `nnue-threat`) は廃止済み。

canonical atomic feature の `cargo build --features <feature>` 直接指定も可能だが、
