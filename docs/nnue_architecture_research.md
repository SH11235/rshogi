...
v1 は attacker side 2 通り、attacked side 2 通り、target class 9 通りなので:
...
**正式な正規化仕様は `docs/threat2a_spec.md` を参照すること。**

Stockfish FullThreats 準拠で **perspective 基準** の正規化を採用:
```text
...
3. side 正規化:
   side_n = 0 (perspective side / friend) / 1 (opposite side / enemy)
```

方向性駒（Pawn, Lance, Knight, Silver, GoldLike）は Stockfish と同様に
色別の attack LUT を持つことで、perspective 基準の正規化と整合させる。
詳細は `docs/threat2a_spec.md`。
