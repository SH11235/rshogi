...
atomic feature (`ls-arch`, `ls-size-1536x16x32`, `ls-ext-psqt`, `ls-ext-threat`,
`nnue-progress-diff` 等) は内部実装の詳細として残し、user は preset edition
feature 経由でのみ触る。canonical 名の直接指定は experimental / upstream ビルド
用途として可能だが非推奨。
...
|---|---|---|
| `layerstack-only` | `ls-arch` | 旧 alias は削除済 (canonical へ完全移行) |
...
互換 alias は移行期間中のみ維持し、edition 軸 + 全 consumer の canonical 移行
完了をもって削除済み。
...
- Phase 1 で既存 feature 名を rename / 分割するため、CI / docs / spsa 関連
  ツールへの波及。互換 alias は移行期間中のみ維持し、移行完了後に削除済み。
