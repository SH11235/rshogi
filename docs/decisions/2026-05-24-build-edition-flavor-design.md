...
- **Status**: Proposed
- **Date**: 2026-05-24
- **設計レビュー**: Codex (REQUEST CHANGES → 反映済) / Claude (APPROVE WITH SUGGESTIONS → 反映済)

## 前提

本 ADR は Rust edition 2024 / stable rustc を前提とする。本 ADR で導入する
`build.rs` の `compile_error!` 多用や Cargo features 数十個の運用は MSRV 1.75
以上で動作 (現プロジェクト依存と同等)。
...
- `edition-universal`: 全 architecture × 全構成 dispatch。slot ゼロの特別 alias。
  内部展開は後述「universal の atomic feature 展開」。
...
```
[mode が必ず 1 個有効]
  not (mode-universal xor mode-family xor mode-specific)
