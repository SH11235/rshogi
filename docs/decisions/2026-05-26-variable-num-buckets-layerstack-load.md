...
| version | bucket layout | engine が取得する `num_buckets` |
|---|---|---|
| `0x7AF32F20` (legacy) | field 無し、暗黙 9 | 定数 `9` を使う (compat path) |
| `0x7AF32F21` (V2) | `arch_str` 直後の `u32` | file から読む |

非 LayerStack (HalfKP / HalfKa 系) は本 ADR の対象外で、`num_buckets` の概念を持たない。

#### tatara `save_quantised` が出力する byte layout (V2 = `0x7AF32F21`)
...
[next 4)  num_buckets                           u32 LE   ← V2 で追加 / legacy には無い
...
- V2:     PSQT bias `[i32; N]` + weights `[i32; ft_in × N]`

Threat / HandCount block 自体は `num_buckets` に依存しない (per-FT feature) ため、
本 ADR で構造変更は不要。Threat と V2 の組み合わせは tatara 側がまだ書き出さない
ため、engine 側でも load 時に「v2 + Threat 同時指定の `.bin`」は未テスト扱いと
する (将来 tatara が対応した時に別 PR で検証する)。
...
| ファイル | 変更内容 |
|---|---|
| `crates/rshogi-core/src/nnue/constants.rs` | `NNUE_VERSION_LAYERSTACK_V2`, `MAX_LAYER_STACK_BUCKETS`, `DEFAULT_NUM_BUCKETS` を追加。`NUM_LAYER_STACK_BUCKETS` は `DEFAULT_NUM_BUCKETS` に rename (§4.1 補足) |
...
| `crates/rshogi-core/src/nnue/network_layer_stacks.rs` | `read_with_options` で legacy / v2 を判別し `num_buckets` を取得、`LayerStacks::read(reader, num_buckets)` に伝播 |
...
- **V2 path**: version `0x7AF32F21` + `num_buckets = 9` を含む in-memory `.bin` を
  構築 (header + 9 個の空 LayerStackBucket) し、load 完了することを確認。
- **同一 net の V1/V2 evaluation 一致**: 同じ FT / PSQT / LayerStacks weight を持つ
  v1 (legacy)・v2 fixture を作り、同じ局面で **`evaluate()` が bit-identical**
  になることを確認 (`num_buckets = 9` ならば binning 式自体は両 path 同じ式に
  なる ─ legacy 側は engine 内部で `progress_sum_to_bucket(sum, 9)` を呼ぶよう
  にしたため)。
...
- **Threat token 混在の reject**: V2 layout だが arch_str に `Threat=` を含む
  fixture を作成し、現状 tatara が未実装である旨を documented behavior として
  test に書く (load 自体は engine 既存パスで読めるが、本 ADR の動作確認範囲外)。
- **PSQT off path**: V2 + `arch_str` に `PSQT=` 無し で load し、PSQT 領域を
  読まずに layerstacks に到達することを確認。
...
1. 本 ADR を merge し、engine v2 を release する。engine v2 は **legacy `.bin` も
   v2 `.bin` も両方 load できる**。
2. tatara 側の version bump (tatara #236) が landing 後、新 `.bin` を engine v2 で
   load して SPRT を実行する。
3. 既存配布 net (legacy `0x7AF32F20`、`v87-400-layerstack.bin` 等) は引き続き
   load 可 (compat path で N=9 として読む)。

旧 engine (本 ADR landing 前) に v2 `.bin` を渡すと version mismatch で reject される
