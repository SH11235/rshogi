...
- エンジン側で残る作業: arch 文字列パーサのバグ修正（`SqrClippedReLU` を
  LayerStacks 信号にしている誤判定、bucket-less SCReLU の活性化検出）。これは
  本 ADR の D5 第 3 項に従い、構造マーカ `AffineTransformSparseInput` と
  ネスト形式の活性化トークンで識別する形に修正する。未実装 feature set
  2 種（`halfka-merged` / `halfka-hm-split`）は #165 Phase A の結果次第。
