...
- エンジン側 arch 文字列パーサのバグ修正（`SqrClippedReLU` 単独を LayerStacks
  ...
  - `parse_feature_set_from_arch`: bucket 無しマーカ = `AffineTransformSparseInput`
    OR `,l2=`（短形式）。これらを持たない arch を、FT_OUT パターン
    (`->1536x2]` / `->768x2]` / `->512x2]`) または `(SqrClippedReLU[` + 独立した
    `(ClippedReLU[` の混在トークンで LayerStacks 判定。
