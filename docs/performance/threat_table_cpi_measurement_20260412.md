...
## update path の fused prev→curr PoC (2026-04-13, `e855bfe5` → revert)

`update_accumulator_with_cache` の非 reset path は `curr.copy_from_slice(prev)`
→ `try_apply_dirty_piece_fast` の 2 段構造。copy_from_slice は 8KB (L1=512) の
memcpy で、fast path success 時には `prev[i] - sub + add` を 1 回の SIMD loop
で直接 curr に書き込めば copy を完全に省略できる、という最適化を試した。

### 実装

- `apply_sub_add_fused_prev_to_curr` / `apply_double_sub_add_fused_prev_to_curr`:
  AVX-512 / AVX2 で prev 読み → curr 書きの片方向 SIMD loop
- `try_apply_dirty_piece_fast_prev_to_curr`: 既存 fast path の prev → curr 版
- `update_accumulator` / `update_accumulator_with_cache` の 2 箇所を変更

### 計測 (sfcache baseline vs sfcache + fused prev→curr)

| 項目 | baseline (72842680) | candidate (e855bfe5) | Δ |
|---|---:|---:|---:|
| avg_nps | 488,281 | 486,783 | **-0.31%** |
| cycles/node | 8,982.5 | 9,012.2 | +0.33% |
| instructions/node | 17,431.0 | 17,325.3 | -0.61% |

**instructions は削減されているが cycles は微悪化**。LTO=fat production build
では `copy_from_slice` と後続の SIMD loop が prefetch overlap で十分効率化され
ており、手動 fuse のメリットが出ない。find_usable_accumulator / leaper fastpath
PoC と同じ「instructions 削減 + CPI 悪化で NPS 効果なし」パターン。

### 判断: revert (`31dfde71`)

効果がないので revert。localized な fused SIMD path は将来 sub/add 数が
多い Threat path に応用できる可能性があるので PoC 記録は残す。
