...
|:----:|------|:-----:|----:|----------|
| 1 | 序盤（9手目） | 30 | 857,819 | 3g3f |
| 2 | 中盤（詰将棋風） | 17 | 773,030 | 3b3c |
| 3 | 終盤（王手飛車） | 18 | 711,810 | N*4d |
| 4 | 終盤（詰み筋） | 19 | 830,240 | G*2h |
| **平均** | - | - | **793,225** | - |
...
| 局面 | 説明 | Depth | NPS | bestmove |
|:----:|------|:-----:|----:|----------|
| 1 | 序盤（9手目） | 24 | 608,066 | 5i6h |
| 2 | 中盤（詰将棋風） | 15 | 596,024 | 8d7d |
| 3 | 終盤（王手飛車） | 15 | 603,769 | S*6a |
| 4 | 終盤（詰み筋） | 17 | 626,696 | G*1c |
| **平均** | - | - | **608,639** | - |
...
|---------|--------:|-------------:|------|
| 本エンジン | 793,225 | 608,639 | `cargo build --release` |
...
| スレッド | NPS | スケール | 効率 |
|---------|----:|--------:|-----:|
| 1 | 608,633 | 1.00x | 100.0% |
| 8 | 2,838,445 | 4.66x | **58.2%** |

### NNUE評価

| スレッド | NPS | スケール | 効率 |
|---------|----:|--------:|-----:|
| 1 | 793,242 | 1.00x | 100.0% |
| 8 | 3,270,609 | 4.12x | **51.5%** |
...
| 順位 | 関数 | CPU% | 状態 | 備考 |
|------|------|------|------|------|
| 1 | `search_node` | 7.51% | - | 探索メインループ |
| 2 | `Network::evaluate` | 6.85% | - | NNUE推論メイン |
| 3 | `refresh_accumulator` | 6.61% | - | NNUE全計算 |
| 4 | `MovePicker::next_move` | 6.24% | 調査完了 | [詳細](#movepicker-調査完了) |
| 5 | `attackers_to_occ` | 5.65% | - | 利き計算 |
| 6 | `do_move_with_prefetch` | 4.00% | - | 指し手実行 |
| 7 | `update_accumulator` | 3.23% | - | Accumulator差分更新 |
| 8 | `append_active_indices` | 3.13% | - | 特徴量インデックス取得 |
| 9 | `update_check_squares` | 2.21% | - | 王手判定マス更新 |
| 10 | `check_move_mate` | 2.21% | - | 1手詰め判定 |
| - | `partial_insertion_sort` | - | 調査完了 | MovePicker内部（PDQSort最適化済み） |

**注**: `getenv`（2.94%）と`_var_os`（1.47%）はlibc/std由来で探索処理には影響しない。

#### NNUE関連の内訳

| 関数 | CPU% | 説明 |
|------|------|------|
| `Network::evaluate` | 6.85% | NNUE推論メイン |
| `refresh_accumulator` | 6.61% | Accumulator全計算（差分更新失敗時） |
| `update_accumulator` | 3.23% | Accumulator差分更新 |
| `append_active_indices` | 3.13% | 特徴量インデックス取得 |
| `append_changed_indices` | 1.96% | 差分インデックス取得 |
| `forward_update_incremental` | 0.67% | 差分更新増分 |
...
| 順位 | 関数 | CPU% | 備考 |
|------|------|------|------|
| 1 | `eval_lv7_like` | 18.97% | Material評価のメイン関数 |
| 2 | `direction_of` | 14.04% | 方向計算 |
| 3 | `attackers_to_occ` | 4.81% | 利き計算 |
| 4 | `search_node` | 4.62% | 探索メインループ |
| 5 | `MovePicker::next_move` | 4.27% | 指し手選択 |
| 6 | `do_move_with_prefetch` | 4.06% | 指し手実行 |
| 7 | `update_long_effect_from` | 3.41% | 長い利き更新 |
| 8 | `qsearch` | 2.27% | 静止探索 |
| 9 | `see_ge` | 2.04% | SEE計算 |
| 10 | `check_move_mate` | 1.93% | 1手詰め判定 |
