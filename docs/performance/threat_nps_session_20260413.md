...
## selfplay 実戦棋力 (v87-400 vs v92-60, sfcache, byoyomi 1000ms, 200局)
...
v92-60 は early checkpoint (160sb まで学習、session では 60 を使用)。
**NPS 18% 劣位でも Elo +33 程度の差に収まっている** のは Threat eval 品質が
補償している証拠。後半 checkpoint で評価すべき。
...
ただし、**v92 の eval 品質は v87 を depth10 で +43 Elo 上回っている** という
過去の計測があり、NPS gap がある程度縮まれば byoyomi でも v87 を超える可能性が
ある。現時点の -13 Elo (v92-140 vs v87-140 @byoyomi) を sfcache 改修で +2-3
Elo 縮めれば、統計的有意差の範囲に入る。
