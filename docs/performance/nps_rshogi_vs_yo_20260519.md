...
| NPS (concurrency 15) | 同一局面・同一 movetime の A/B（32 コア機、15 並列 single-thread）で aggregate avg_nps **358K → 383–385K（+6.8〜7.4%、2 回計測）**。探索深度は同一 |

PSS 計測が機構の直接証明（重み常駐 N×→1×）。NPS は同一条件 A/B で +7% 前後の
スループット改善を確認（本記録冒頭の −32% 崩落のうち、重み重複コピーに起因する
帯域競合ぶんを回復。TT 等の他要因の競合は残る）。
