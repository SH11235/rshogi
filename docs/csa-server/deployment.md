...
> で grep / filter できる JSON 形式への移行は別 PR で導入予定。
...
### 6.4 本 runbook を構成する merged PR と未実装の改善ポイント

本 runbook と関連実装の構築は以下の merged PR で進めてきた:

| PR | 内容 |
|---|---|
| [#503](https://github.com/SH11235/rshogi/pull/503) | CI に wasm32 ビルド検査 job を追加 |
| [#505](https://github.com/SH11235/rshogi/pull/505) | `wrangler.toml.example` と `ConfigKeys` の整合性 test |
| [#507](https://github.com/SH11235/rshogi/pull/507) | 自動 deploy workflow + 本 runbook の初版 |
| [#508](https://github.com/SH11235/rshogi/pull/508) | `wrangler` を `package.json` devDependency 化 + Vite+ ローカル管理 |

未実装の改善ポイント（順序は要件発生に応じて別 PR で着手）:

- **Miniflare smoke E2E ハーネス**: `wrangler dev` 経由で WS Upgrade →
  対局成立 → 終局 → R2 棋譜出力までを自動 smoke test 化する
- **staging 環境 + production の `workflow_dispatch` only 化**: 現状 main
  push で production が deploy されるため、shogi engine 改修だけでも deploy
  が走る。staging を auto-deploy 先にして runtime regression を smoke 検出し、
  production は意図的 dispatch だけに絞る canary パターンへ移行する
