...
| 23.4 | 自動 deploy workflow + 本 runbook の初版（PR #507）|
| 23.5 | `wrangler` を `package.json` devDependency 化 + Vite+ ローカル管理（PR #508）|
| 23.6 | `ADMIN_HANDLE` を Cloudflare secret に格上げ + production toml 整合性 test |
| 23.7 | staging 環境を立てて production を `workflow_dispatch` only 化（未着手）|
| 23.8 | wrangler 3 → 4 系への migration（未着手）|
