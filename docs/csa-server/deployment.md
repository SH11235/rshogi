...
| ローカル環境 | Node.js 20+ / pnpm (`package.json` の `packageManager` で pin、corepack 経由で自動取得) / Rust toolchain (`rust-toolchain.toml` で pin) + `wasm32-unknown-unknown` target |
...
ローカルに install する経路に統一しているので、`pnpm install` だけでローカルと
CI のバージョンが一致する。
...
# Node 側 (package.json で wrangler を pin、global install 不要)
corepack enable     # 初回のみ。`packageManager` field の pnpm を有効化
cd crates/rshogi-csa-server-workers
pnpm install
```

以降 `wrangler` は `pnpm exec wrangler ...` または scripts (`pnpm deploy:prod`
等) 経由で呼び出す。`pnpm add -g wrangler` などの global install は不要。
...
CLI からも確認したい場合は §3.2 で `pnpm exec wrangler login` を済ませた後で:
...
```bash
cd crates/rshogi-csa-server-workers
pnpm exec wrangler r2 bucket create rshogi-csa-kifu-prod
pnpm exec wrangler r2 bucket create rshogi-csa-floodgate-history-prod
```
...
- ⚠️ **`[vars] ADMIN_HANDLE` を必ず運用上の admin handle に変更する**。デフォルト
  値 `"admin"` は一般的なハンドル名のため、誰かが `LOGIN admin+pwd` で接続して
  `%%SETBUOY` / `%%DELETEBUOY` 等の運営権限コマンドを実行できてしまうリスクが
  ある。本番 deploy 前の必須変更項目
...
`pnpm deploy:prod` は `package.json` で
...
```bash
cd crates/rshogi-csa-server-workers
pnpm exec wrangler rollback --config wrangler.production.toml
```
...
```bash
pnpm exec wrangler deployments list --config wrangler.production.toml
# Version ID をコピー

pnpm exec wrangler rollback <version-id> --config wrangler.production.toml
```
...
3. **不整合状態の場合は §5.1 / §5.2 の手順で安定 version に rollback**
   - 直前の安定 version に戻すなら §5.1（`pnpm exec wrangler rollback --config ...`）
   - 特定の安定 version に戻すなら §5.2（`pnpm exec wrangler deployments list` で ID を
     確認して `pnpm exec wrangler rollback <version-id> --config ...`）
...
`pnpm tail:prod` は `package.json` で
...
- `wrangler.production.toml` の `bucket_name` が誤っている、または bucket が
  Cloudflare 上に未作成
- 対処: §2.1 の `pnpm exec wrangler r2 bucket create` を実行、または toml 側を修正
...
- 対処: §6.1 の `pnpm tail:prod` で wrangler tail を見ながら client から接続して
  4xx/5xx を観測
