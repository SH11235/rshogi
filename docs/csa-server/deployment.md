...
### 3.4 web client 化時に `WS_ALLOWED_ORIGINS` を追加する
...
#### 3.4.1 標準フロー (新規 web client URL を追加)
...
#### 3.4.4 incident response (allowlist 緊急停止)
...
2. CI 通過後 `gh pr merge --squash` で main に入れる。
   ※ CI / Required reviewers 待ちが許容できない緊急度の場合は §5.1 の
   `wrangler rollback` を **先に** 実行してブラウザ接続を即時遮断し、その後 (a)
   経路で repo state を同期させる (§3.4.5(b) と同じ手順)。
3. §4.2 の `workflow_dispatch (target=production)` で即時 deploy (通常の dispatch 経路と同じ)。
4. §3.4.3 のとおり既存対局は切れず、新規ブラウザ Upgrade だけが 403 になる。ネイティブ
   CSA クライアントはこの incident response の影響を受けないため、対局運用の中核は継続できる。
5. 原因解消後、§3.4.1 の標準フローで Origin を再追加する。

#### 3.4.5 rollback (新 Origin が問題を起こした場合)
...
**(b) `wrangler rollback` 経路 (§5.1 参照)**: Cloudflare 側の前 version snapshot に
...
いずれの経路も §3.4.3 の "削除" 挙動なので既存対局は切れない。新規接続だけが旧 allowlist
基準に戻る。
...
Issue #591 の partial fix (新規対局で `Reconnect_Token:` 拡張行の配布を
...
protocol の本格有効化、Issue #591 の Expected 完了条件 (1)) は **followup-C
...
5. followup-E (Issue #591 完了条件): `RECONNECT_GRACE_SECONDS=30` +
...
Issue #601 で配線済。staging は canary 用途 (`main` push 自動 deploy) のため
drain 対象外。
...
3. 真に対局中なら deadline まで待つ。stuck (AGREE 待ち TTL 未配線、Issue #600 対象)
