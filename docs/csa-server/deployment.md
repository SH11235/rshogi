...
- staging / production の Worker は別 `name` を持つので、Durable Object namespace
  と R2 bucket が独立する（互いの状態には触れない）。
...
| token 名（例） | 用途 |
|---|---|
| `rshogi-csa-server-workers-staging-deploy` | staging environment の `CLOUDFLARE_API_TOKEN` に登録 |
| `rshogi-csa-server-workers-production-deploy` | production environment の `CLOUDFLARE_API_TOKEN` に登録 |
...
`staging` / `production` の 2 つの GitHub Environment を作成し、それぞれに
secret と variable を登録する。Environment ベースにすることで、deploy job 側で
`environment: staging` / `environment: production` を宣言した時に **その環境
に紐付いた値だけ** が注入される（cross-env の secret 取り違えが発生しない）。
...
|---|---|
| Name | `staging` |
| Deployment branches | Selected branches → `main` のみ許可 |
...
| Name | `production` |
| Deployment branches | Selected branches → `main` のみ許可 |
| Required reviewers | （運用方針に応じて。設定しておくと `workflow_dispatch` 起動時に承認 step が挟まる） |
...
同様に smoke check し、`/health` URL を production environment の variable に登録する。
...
| `wrangler.staging.toml` | CI 自動 deploy 用 staging 設定 | **Tracked**（§3.1 参照）| 公開値（`PRODUCTION_VARS_KEYS`）のみ。`LOCAL_DEV_ONLY_VARS_KEYS` は **書かない**（§2.5 で secret 化） |
| `wrangler.production.toml` | CI dispatch deploy 用 production 設定 | **Tracked**（§3.2 参照）| 公開値（`PRODUCTION_VARS_KEYS`）のみ。`LOCAL_DEV_ONLY_VARS_KEYS` は **書かない**（§2.5 で secret 化） |
