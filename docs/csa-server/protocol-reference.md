...
opt-in flag が gate する具体的なフィールド集合 (`FloodgateFeatureIntent`) と
要求 → opt-in 検証ロジックの一次ソースは [`crates/rshogi-csa-server/src/config.rs`](../../crates/rshogi-csa-server/src/config.rs)
(`FloodgateFeatureIntent` / `validate_floodgate_feature_gate` /
`floodgate_intent_from_config`)。
