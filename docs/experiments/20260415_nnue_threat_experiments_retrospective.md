...
### 本日実施した HandThreat-Def 実験の archive

```bash
# rshogi
git fetch --tags
git checkout archive/hand-threat-defensive
# → HandThreat-Def 実装時点の rshogi state (commit 3e1f1b0d)
# 主要ファイル: crates/rshogi-core/src/nnue/hand_threat_features.rs
#              crates/rshogi-core/Cargo.toml (feature flag 定義)

# bullet-shogi
cd /path/to/bullet-shogi
git fetch --tags
git checkout archive/hand-threat-defensive
# → HandThreat-Def 実装時点の bullet-shogi state (commit ea48f67)
# 主要ファイル: crates/bullet_lib/src/game/inputs/shogi_halfka_hm_hand_threat_defensive.rs
#              crates/bullet_lib/src/game/inputs.rs (trait 拡張)
#              crates/bullet_lib/src/value/loader.rs (非対称 loader)
```
