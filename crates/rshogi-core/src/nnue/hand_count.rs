...
//!
//! `HAND_COUNT_DIMS` 定数は `nnue-hand-count-dense` feature 無効ビルドでも
//! `LayerStackBucket` のメソッドシグネチャから参照できるよう常に公開する。
//! 実際のロード・評価パスは feature 有効時のみ有効化される。

/// HandCount Dense 補助入力の次元数（`[stm 7 種 + nstm 7 種] = 14`）
#[allow(dead_code)] // feature 無効時は参照されないが、`layer_stacks.rs` からの cfg
// 経由でのみ使われるため warnings 防止に allow を付ける
pub const HAND_COUNT_DIMS: usize = 14;

#[cfg(feature = "nnue-hand-count-dense")]
pub use imp::extract_hand_count;

#[cfg(feature = "nnue-hand-count-dense")]
mod imp {
    use super::HAND_COUNT_DIMS;
