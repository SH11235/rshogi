...
mod tests {
    ...
    fn valid_init_forward<A: FtActivation, const INPUT: usize>() {
        ...
        let network = NetworkHalfKaSplit::<32, 64, INPUT, 32, 32, A> {
            feature_transformer: FeatureTransformerHalfKaSplit {
                biases: vec![0; 32],
                weights: AlignedBox::new_zeroed(1),
            },
            l1: AffineTransformHalfKaSplit::read(&mut &dense_bytes(INPUT, 32)[..]).unwrap(),
            l2: AffineTransformHalfKaSplit::read(&mut &dense_bytes(32, 32)[..]).unwrap(),
            output: AffineTransformHalfKaSplit::read(&mut &dense_bytes(32, 1)[..]).unwrap(),
            fv_scale: 16,
            qa: 127,
            _activation: PhantomData,
        };
        let mut acc = AccumulatorHalfKaSplit::<32>::new();
        for i in 0..32 {
