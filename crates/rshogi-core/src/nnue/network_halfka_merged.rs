...
//! NetworkHalfKaMerged - const generics ベースの HalfKaMerged ネットワーク統一実装
//!
//! HalfKaMerged 特徴量を使用し、L1/L2/L3 のサイズと活性化関数を型パラメータで切り替え可能にした実装。
//!
//! # 設計
//!
//! ```text
...
//! | 型エイリアス | L1 | L2 | L3 | 活性化 |
//! |-------------|------|-----|-----|--------|
//! | HalfKaMerged256CReLU | 256 | 32 | 32 | CReLU |
//! | HalfKaMerged256SCReLU | 256 | 32 | 32 | SCReLU |
//! | HalfKaMerged256Pairwise | 256 | 32 | 32 | PairwiseCReLU |
//! | HalfKaMerged512CReLU | 512 | 8 | 96 | CReLU |
//! | HalfKaMerged512SCReLU | 512 | 8 | 96 | SCReLU |
//! | HalfKaMerged512Pairwise | 512 | 8 | 96 | PairwiseCReLU |
//! | HalfKaMerged512_32_32CReLU | 512 | 32 | 32 | CReLU |
//! | HalfKaMerged512_32_32SCReLU | 512 | 32 | 32 | SCReLU |
//! | HalfKaMerged512_32_32Pairwise | 512 | 32 | 32 | PairwiseCReLU |
//! | HalfKaMerged1024CReLU | 1024 | 8 | 96 | CReLU |
//! | HalfKaMerged1024SCReLU | 1024 | 8 | 96 | SCReLU |
//! | HalfKaMerged1024Pairwise | 1024 | 8 | 96 | PairwiseCReLU |
//! | HalfKaMerged1024_8_32CReLU | 1024 | 8 | 32 | CReLU |
//! | HalfKaMerged1024_8_32SCReLU | 1024 | 8 | 32 | SCReLU |
//! | HalfKaMerged1024_8_32Pairwise | 1024 | 8 | 32 | PairwiseCReLU |
//!
//! # 特徴量
//!
...
impl<const L1: usize> AccumulatorHalfKaMerged<L1> {
    /// 新規作成
    pub fn new() -> Self {
        Self {
...
impl<const L1: usize> Default for AccumulatorHalfKaMerged<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// AccumulatorStackHalfKaMerged - アキュムレータスタック
// =============================================================================

/// スタックエントリ
pub struct AccumulatorEntryHalfKaMerged<const L1: usize> {
    pub accumulator: AccumulatorHalfKaMerged<L1>,
    pub dirty_piece: DirtyPiece,
    pub previous: Option<usize>,
}

/// アキュムレータスタック
pub struct AccumulatorStackHalfKaMerged<const L1: usize> {
    entries: Vec<AccumulatorEntryHalfKaMerged<L1>>,
    current_idx: usize,
}

impl<const L1: usize> AccumulatorStackHalfKaMerged<L1> {
    /// 新規作成
    pub fn new() -> Self {
        let mut entries = Vec::with_capacity(128);
        entries.push(AccumulatorEntryHalfKaMerged {
    ...
    /// 現在のエントリを取得
    pub fn current(&self) -> &AccumulatorEntryHalfKaMerged<L1> {
        &self.entries[self.current_idx]
    }

    /// 現在のエントリを取得（可変）
    pub fn current_mut(&mut self) -> &mut AccumulatorEntryHalfKaMerged<L1> {
    ...
    #[inline]
    pub fn top(&self) -> &AccumulatorHalfKaMerged<L1> {
    ...
    /// `define_l1_variants!` マクロから使用される。
    #[inline]
    pub fn top_mut(&mut self) -> &mut AccumulatorHalfKaMerged<L1> {
    ...
    pub fn top_and_source(
        &mut self,
        source_idx: usize,
    ) -> (&mut AccumulatorHalfKaMerged<L1>, &AccumulatorHalfKaMerged<L1>) {
    ...
    /// 指定インデックスのエントリを取得
    pub fn entry_at(&self, idx: usize) -> &AccumulatorEntryHalfKaMerged<L1> {
        &self.entries[idx]
    }

    /// 指定インデックスのエントリを取得（可変）
    pub fn entry_at_mut(&mut self, idx: usize) -> &mut AccumulatorEntryHalfKaMerged<L1> {
    ...
    pub fn get_prev_and_current_accumulators(
        &mut self,
        prev_idx: usize,
    ) -> (&AccumulatorHalfKaMerged<L1>, &mut AccumulatorHalfKaMerged<L1>) {
        let current_idx = self.current_idx;
...
impl<const L1: usize> Default for AccumulatorStackHalfKaMerged<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// FeatureTransformerHalfKaMerged - const generics 版 Feature Transformer
// =============================================================================

/// HalfKaMerged Feature Transformer
pub struct FeatureTransformerHalfKaMerged<const L1: usize> {
    /// バイアス [L1]
    pub biases: Vec<i16>,
...
impl<const L1: usize> FeatureTransformerHalfKaMerged<L1> {
    ...
    pub(crate) fn share_weights(&mut self) {
        super::shared_weights::try_share(&mut self.weights, "FT weights (HalfKaMerged)");
    }

    /// ファイルから読み込み
    pub fn read<R: Read>(reader: &mut R) -> io::Result<Self> {
        let input_dim = HALFKA_MERGED_DIMENSIONS;
    ...
    /// Accumulatorをリフレッシュ
    pub fn refresh_accumulator(&self, pos: &Position, acc: &mut AccumulatorHalfKaMerged<L1>) {
    ...
    pub fn update_accumulator(
        ...
        for perspective in [Color::Black, Color::White] {
            ...
            if reset {
                ...
                let active_indices =
                    HalfKaMergedFeatureSet::collect_active_indices(pos, perspective);
                for index in active_indices.iter() {
                ...
                <HalfKaMerged as Feature>::append_changed_indices(
                    dirty_piece,
                    perspective,
    ...
    pub fn forward_update_incremental(
        &self,
        pos: &Position,
        stack: &mut AccumulatorStackHalfKaMerged<L1>,
        source_idx: usize,
    ) -> bool {
    ...
    pub fn transform_raw(
        &self,
        acc: &AccumulatorHalfKaMerged<L1>,
        side_to_move: Color,
        output: &mut [i16],
    ) {
...
impl<const INPUT: usize, const OUTPUT: usize> AffineTransformHalfKaMerged<INPUT, OUTPUT> {
    /// パディング済み入力次元（32の倍数）
...
// =============================================================================
// NetworkHalfKaMerged - const generics 版統一ネットワーク
// =============================================================================

/// HalfKaMerged ネットワーク（const generics 版）
///
/// # 型パラメータ
/// - `L1`: FT出力次元（片側）
...
pub struct NetworkHalfKaMerged<
    const L1: usize,
    const FT_OUT: usize,
...
> {
    ...
    pub feature_transformer: FeatureTransformerHalfKaMerged<L1>,
    /// 隠れ層1: L1_INPUT → L2
    pub l1: AffineTransformHalfKaMerged<L1_INPUT, L2>,
    /// 隠れ層2: L2 → L3
    pub l2: AffineTransformHalfKaMerged<L2, L3>,
    /// 出力層: L3 → 1
    pub output: AffineTransformHalfKaMerged<L3, 1>,
    /// 評価値スケーリング係数
    pub fv_scale: i32,
    /// QA値（クリッピング閾値）
    pub qa: i16,
...
impl<
    ...
    const L3: usize,
    A: FtActivation,
> NetworkHalfKaMerged<L1, FT_OUT, L1_INPUT, L2, L3, A>
{
    /// コンパイル時制約
    ///
    ...
    pub fn read<R: Read + Seek>(reader: &mut R) -> io::Result<Self> {
        ...
        if arch_str.contains("Factorizer") {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "Unsupported model format: factorized (non-coalesced) HalfKA_merged^ model detected.\n\
                     This engine only supports coalesced models (131,949 dimensions).\n\
        ...
        let mut feature_transformer = FeatureTransformerHalfKaMerged::read(reader)?;

        // FC layers ハッシュ
        reader.read_exact(&mut buf4)?;

        // l1: L1*2 → L2
        let l1 = AffineTransformHalfKaMerged::read(reader)?;

        // l2: L2 → L3
        let l2 = AffineTransformHalfKaMerged::read(reader)?;

        // output: L3 → 1
        let output = AffineTransformHalfKaMerged::read(reader)?;
    ...
    pub fn evaluate(&self, pos: &Position, acc: &AccumulatorHalfKaMerged<L1>) -> Value {
        ...
        for (i, &v) in l1_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L1 output[{i}] = {v} is out of expected range (NetworkHalfKaMerged<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        for (i, &v) in l2_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L2 output[{i}] = {v} is out of expected range (NetworkHalfKaMerged<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        debug_assert!(
            eval.abs() < 50_000,
            "Final evaluation {eval} is out of expected range (NetworkHalfKaMerged<{}, {}, {}, {}>). Raw output: {}",
            L1,
            L2,
            L3,
            A::name(),
    ...
    /// 新しい Accumulator を作成
    pub fn new_accumulator(&self) -> AccumulatorHalfKaMerged<L1> {
        AccumulatorHalfKaMerged::new()
    }

    /// 新しい AccumulatorStack を作成
    pub fn new_accumulator_stack(&self) -> AccumulatorStackHalfKaMerged<L1> {
        AccumulatorStackHalfKaMerged::new()
    }

    /// アーキテクチャ名を取得
    pub fn architecture_name(&self) -> String {
        format!("HalfKA_merged^{}x2-{}-{}-{}", L1, L2, L3, A::name())
    }
}
...
mod tests {
    ...
    fn test_accumulator_halfka_512() {
        let acc = AccumulatorHalfKaMerged::<512>::new();
        assert_eq!(acc.accumulation[0].0.len(), 512);
    }

    #[test]
    fn test_accumulator_halfka_1024() {
        let acc = AccumulatorHalfKaMerged::<1024>::new();
        assert_eq!(acc.accumulation[0].0.len(), 1024);
    }

    #[test]
    fn test_padded_input() {
        assert_eq!(AffineTransformHalfKaMerged::<8, 96>::PADDED_INPUT, 32);
        assert_eq!(AffineTransformHalfKaMerged::<32, 96>::PADDED_INPUT, 32);
        assert_eq!(AffineTransformHalfKaMerged::<33, 96>::PADDED_INPUT, 64);
        assert_eq!(AffineTransformHalfKaMerged::<96, 1>::PADDED_INPUT, 96);
        assert_eq!(AffineTransformHalfKaMerged::<1024, 8>::PADDED_INPUT, 1024);
        assert_eq!(AffineTransformHalfKaMerged::<2048, 8>::PADDED_INPUT, 2048);
    }
    ...
    fn test_parse_qa_from_arch() {
        ...
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKA_merged[131949->512x2],fv_scale=16,l1_input=1024,l2=8,l3=96,qa=255,qb=64"
            ),
            Some(255)
        );
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKA_merged[131949->512x2],fv_scale=16,l1_input=1024,l2=8,l3=96,qa=127,qb=64"
            ),
            Some(127)
        );
        ...
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKA_merged[131949->512x2],Network=AffineTransform[1<-96]"
            ),
            None
        );
    }
    ...
    fn test_type_aliases() {
        ...
        fn _check_halfka_256_crelu(_: HalfKaMerged256CReLU) {}
        fn _check_halfka_512_crelu(_: HalfKaMerged512CReLU) {}
        fn _check_halfka_1024_crelu(_: HalfKaMerged1024CReLU) {}
    }
}
