...
//! | 型エイリアス | L1 | L2 | L3 | 活性化 |
//! |-------------|------|-----|-----|--------|
//! | HalfKaHmSplit256CReLU | 256 | 32 | 32 | CReLU |
//! | HalfKaHmSplit256SCReLU | 256 | 32 | 32 | SCReLU |
//! | HalfKaHmSplit256Pairwise | 256 | 32 | 32 | PairwiseCReLU |
//! | HalfKaHmSplit512CReLU | 512 | 8 | 96 | CReLU |
//! | HalfKaHmSplit512SCReLU | 512 | 8 | 96 | SCReLU |
//! | HalfKaHmSplit512Pairwise | 512 | 8 | 96 | PairwiseCReLU |
//! | HalfKaHmSplit512_32_32CReLU | 512 | 32 | 32 | CReLU |
//! | HalfKaHmSplit512_32_32SCReLU | 512 | 32 | 32 | SCReLU |
//! | HalfKaHmSplit512_32_32Pairwise | 512 | 32 | 32 | PairwiseCReLU |
//! | HalfKaHmSplit1024CReLU | 1024 | 8 | 96 | CReLU |
//! | HalfKaHmSplit1024SCReLU | 1024 | 8 | 96 | SCReLU |
//! | HalfKaHmSplit1024Pairwise | 1024 | 8 | 96 | PairwiseCReLU |
//! | HalfKaHmSplit1024_8_32CReLU | 1024 | 8 | 32 | CReLU |
//! | HalfKaHmSplit1024_8_32SCReLU | 1024 | 8 | 32 | SCReLU |
//! | HalfKaHmSplit1024_8_32Pairwise | 1024 | 8 | 32 | PairwiseCReLU |
//!
//! # 特徴量
//!
...
use super::features::{Feature, FeatureSet, HalfKaHmSplit, HalfKaHmSplitFeatureSet};
...
impl<const L1: usize> AccumulatorHalfKaHmSplit<L1> {
    /// 新規作成
    pub fn new() -> Self {
        Self {
...
impl<const L1: usize> Default for AccumulatorHalfKaHmSplit<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// AccumulatorStackHalfKaHmSplit - アキュムレータスタック
// =============================================================================

/// スタックエントリ
pub struct AccumulatorEntryHalfKaHmSplit<const L1: usize> {
    pub accumulator: AccumulatorHalfKaHmSplit<L1>,
    pub dirty_piece: DirtyPiece,
    pub previous: Option<usize>,
}

/// アキュムレータスタック
pub struct AccumulatorStackHalfKaHmSplit<const L1: usize> {
    entries: Vec<AccumulatorEntryHalfKaHmSplit<L1>>,
    current_idx: usize,
}

impl<const L1: usize> AccumulatorStackHalfKaHmSplit<L1> {
    /// 新規作成
    pub fn new() -> Self {
        let mut entries = Vec::with_capacity(128);
        entries.push(AccumulatorEntryHalfKaHmSplit {
            accumulator: AccumulatorHalfKaHmSplit::new(),
    ...
    /// 現在のエントリを取得
    pub fn current(&self) -> &AccumulatorEntryHalfKaHmSplit<L1> {
        &self.entries[self.current_idx]
    }

    /// 現在のエントリを取得（可変）
    pub fn current_mut(&mut self) -> &mut AccumulatorEntryHalfKaHmSplit<L1> {
    ...
    #[inline]
    pub fn top(&self) -> &AccumulatorHalfKaHmSplit<L1> {
    ...
    /// `define_l1_variants!` マクロから使用される。
    #[inline]
    pub fn top_mut(&mut self) -> &mut AccumulatorHalfKaHmSplit<L1> {
    ...
    pub fn top_and_source(
        &mut self,
        source_idx: usize,
    ) -> (&mut AccumulatorHalfKaHmSplit<L1>, &AccumulatorHalfKaHmSplit<L1>) {
    ...
    pub fn push(&mut self, dirty_piece: DirtyPiece) {
        ...
        self.entries.push(AccumulatorEntryHalfKaHmSplit {
            accumulator: AccumulatorHalfKaHmSplit::new(),
            dirty_piece,
    ...
    /// 指定インデックスのエントリを取得
    pub fn entry_at(&self, idx: usize) -> &AccumulatorEntryHalfKaHmSplit<L1> {
        &self.entries[idx]
    }

    /// 指定インデックスのエントリを取得（可変）
    pub fn entry_at_mut(&mut self, idx: usize) -> &mut AccumulatorEntryHalfKaHmSplit<L1> {
    ...
    pub fn get_prev_and_current_accumulators(
        &mut self,
        prev_idx: usize,
    ) -> (&AccumulatorHalfKaHmSplit<L1>, &mut AccumulatorHalfKaHmSplit<L1>) {
        let current_idx = self.current_idx;
...
impl<const L1: usize> Default for AccumulatorStackHalfKaHmSplit<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// FeatureTransformerHalfKaHmSplit - const generics 版 Feature Transformer
// =============================================================================

/// HalfKaHmSplit Feature Transformer
pub struct FeatureTransformerHalfKaHmSplit<const L1: usize> {
    /// バイアス [L1]
    pub biases: Vec<i16>,
...
impl<const L1: usize> FeatureTransformerHalfKaHmSplit<L1> {
    ...
    pub(crate) fn share_weights(&mut self) {
        super::shared_weights::try_share(&mut self.weights, "FT weights (HalfKaHmSplit)");
    }

    /// ファイルから読み込み
    pub fn read<R: Read>(reader: &mut R) -> io::Result<Self> {
        let input_dim = HALFKA_HM_SPLIT_DIMENSIONS;
    ...
    /// Accumulatorをリフレッシュ
    pub fn refresh_accumulator(&self, pos: &Position, acc: &mut AccumulatorHalfKaHmSplit<L1>) {
    ...
    pub fn update_accumulator(
        ...
        acc: &mut AccumulatorHalfKaHmSplit<L1>,
        prev_acc: &AccumulatorHalfKaHmSplit<L1>,
    ) {
        for perspective in [Color::Black, Color::White] {
            let p = perspective as usize;
            let reset = HalfKaHmSplitFeatureSet::needs_refresh(dirty_piece, perspective);
            ...
            if reset {
                ...
                <HalfKaHmSplit as Feature>::append_changed_indices(
                    dirty_piece,
                    perspective,
    ...
    pub fn forward_update_incremental(
        &self,
        pos: &Position,
        stack: &mut AccumulatorStackHalfKaHmSplit<L1>,
        source_idx: usize,
    ) -> bool {
    ...
    pub fn transform_raw(
        &self,
        acc: &AccumulatorHalfKaHmSplit<L1>,
        side_to_move: Color,
        output: &mut [i16],
    ) {
...
impl<const INPUT: usize, const OUTPUT: usize> AffineTransformHalfKaHmSplit<INPUT, OUTPUT> {
    /// パディング済み入力次元（32の倍数）
...
// =============================================================================
// NetworkHalfKaHmSplit - const generics 版統一ネットワーク
// =============================================================================

/// HalfKaHmSplit ネットワーク（const generics 版）
///
/// # 型パラメータ
/// - `L1`: FT出力次元（片側）
...
pub struct NetworkHalfKaHmSplit<
    const L1: usize,
    const FT_OUT: usize,
...
> {
    ...
    pub feature_transformer: FeatureTransformerHalfKaHmSplit<L1>,
    /// 隠れ層1: L1_INPUT → L2
    pub l1: AffineTransformHalfKaHmSplit<L1_INPUT, L2>,
    /// 隠れ層2: L2 → L3
    pub l2: AffineTransformHalfKaHmSplit<L2, L3>,
    /// 出力層: L3 → 1
    pub output: AffineTransformHalfKaHmSplit<L3, 1>,
    /// 評価値スケーリング係数
    pub fv_scale: i32,
    /// QA値（クリッピング閾値）
    pub qa: i16,
...
impl<
    ...
    const L3: usize,
    A: FtActivation,
> NetworkHalfKaHmSplit<L1, FT_OUT, L1_INPUT, L2, L3, A>
{
    /// コンパイル時制約
    ///
    ...
    pub fn read<R: Read + Seek>(reader: &mut R) -> io::Result<Self> {
        ...
        let mut feature_transformer = FeatureTransformerHalfKaHmSplit::read(reader)?;

        // FC layers ハッシュ
        reader.read_exact(&mut buf4)?;

        // l1: L1*2 → L2
        let l1 = AffineTransformHalfKaHmSplit::read(reader)?;

        // l2: L2 → L3
        let l2 = AffineTransformHalfKaHmSplit::read(reader)?;

        // output: L3 → 1
        let output = AffineTransformHalfKaHmSplit::read(reader)?;
    ...
    pub fn evaluate(&self, pos: &Position, acc: &AccumulatorHalfKaHmSplit<L1>) -> Value {
        ...
        for (i, &v) in l1_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L1 output[{i}] = {v} is out of expected range (NetworkHalfKaHmSplit<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        for (i, &v) in l2_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L2 output[{i}] = {v} is out of expected range (NetworkHalfKaHmSplit<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        debug_assert!(
            eval.abs() < 50_000,
            "Final evaluation {eval} is out of expected range (NetworkHalfKaHmSplit<{}, {}, {}, {}>). Raw output: {}",
            L1,
            L2,
            L3,
            A::name(),
    ...
    /// 新しい Accumulator を作成
    pub fn new_accumulator(&self) -> AccumulatorHalfKaHmSplit<L1> {
        AccumulatorHalfKaHmSplit::new()
    }

    /// 新しい AccumulatorStack を作成
    pub fn new_accumulator_stack(&self) -> AccumulatorStackHalfKaHmSplit<L1> {
        AccumulatorStackHalfKaHmSplit::new()
    }
...
mod tests {
    ...
    fn valid_init_forward<A: FtActivation, const INPUT: usize>() {
        ...
        let network = NetworkHalfKaHmSplit::<32, 64, INPUT, 32, 32, A> {
            feature_transformer: FeatureTransformerHalfKaHmSplit {
                biases: vec![0; 32],
                weights: AlignedBox::new_zeroed(1),
            },
            l1: AffineTransformHalfKaHmSplit::read(&mut &dense_bytes(INPUT, 32)[..]).unwrap(),
            l2: AffineTransformHalfKaHmSplit::read(&mut &dense_bytes(32, 32)[..]).unwrap(),
            output: AffineTransformHalfKaHmSplit::read(&mut &dense_bytes(32, 1)[..]).unwrap(),
            fv_scale: 16,
            qa: 127,
            _activation: PhantomData,
        };
        let mut acc = AccumulatorHalfKaHmSplit::<32>::new();
        for i in 0..32 {
    ...
    fn test_accumulator_halfka_512() {
        let acc = AccumulatorHalfKaHmSplit::<512>::new();
        assert_eq!(acc.accumulation[0].0.len(), 512);
    }

    #[test]
    fn test_accumulator_halfka_1024() {
        let acc = AccumulatorHalfKaHmSplit::<1024>::new();
        assert_eq!(acc.accumulation[0].0.len(), 1024);
    }

    #[test]
    fn test_padded_input() {
        assert_eq!(AffineTransformHalfKaHmSplit::<8, 96>::PADDED_INPUT, 32);
        assert_eq!(AffineTransformHalfKaHmSplit::<32, 96>::PADDED_INPUT, 32);
        assert_eq!(AffineTransformHalfKaHmSplit::<33, 96>::PADDED_INPUT, 64);
        assert_eq!(AffineTransformHalfKaHmSplit::<96, 1>::PADDED_INPUT, 96);
        assert_eq!(AffineTransformHalfKaHmSplit::<1024, 8>::PADDED_INPUT, 1024);
        assert_eq!(AffineTransformHalfKaHmSplit::<2048, 8>::PADDED_INPUT, 2048);
    }
    ...
    fn test_type_aliases() {
        ...
        fn _check_halfka_256_crelu(_: HalfKaHmSplit256CReLU) {}
        fn _check_halfka_512_crelu(_: HalfKaHmSplit512CReLU) {}
        fn _check_halfka_1024_crelu(_: HalfKaHmSplit1024CReLU) {}
    }
}
