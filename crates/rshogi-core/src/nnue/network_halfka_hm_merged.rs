...
impl<const L1: usize> AccumulatorHalfKaHmMerged<L1> {
    /// 新規作成
    pub fn new() -> Self {
        Self {
...
impl<const L1: usize> Default for AccumulatorHalfKaHmMerged<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// AccumulatorStackHalfKaHmMerged - アキュムレータスタック
// =============================================================================

/// スタックエントリ
pub struct AccumulatorEntryHalfKaHmMerged<const L1: usize> {
    pub accumulator: AccumulatorHalfKaHmMerged<L1>,
    pub dirty_piece: DirtyPiece,
    pub previous: Option<usize>,
}

/// アキュムレータスタック
pub struct AccumulatorStackHalfKaHmMerged<const L1: usize> {
    entries: Vec<AccumulatorEntryHalfKaHmMerged<L1>>,
    current_idx: usize,
}

impl<const L1: usize> AccumulatorStackHalfKaHmMerged<L1> {
    /// 新規作成
    pub fn new() -> Self {
        let mut entries = Vec::with_capacity(128);
        entries.push(AccumulatorEntryHalfKaHmMerged {
            accumulator: AccumulatorHalfKaHmMerged::new(),
    ...
    /// 現在のエントリを取得
    pub fn current(&self) -> &AccumulatorEntryHalfKaHmMerged<L1> {
        &self.entries[self.current_idx]
    }

    /// 現在のエントリを取得（可変）
    pub fn current_mut(&mut self) -> &mut AccumulatorEntryHalfKaHmMerged<L1> {
    ...
    #[inline]
    pub fn top(&self) -> &AccumulatorHalfKaHmMerged<L1> {
    ...
    /// `define_l1_variants!` マクロから使用される。
    #[inline]
    pub fn top_mut(&mut self) -> &mut AccumulatorHalfKaHmMerged<L1> {
    ...
    pub fn top_and_source(
        &mut self,
        source_idx: usize,
    ) -> (&mut AccumulatorHalfKaHmMerged<L1>, &AccumulatorHalfKaHmMerged<L1>) {
    ...
    pub fn push(&mut self, dirty_piece: DirtyPiece) {
        ...
        self.entries.push(AccumulatorEntryHalfKaHmMerged {
            accumulator: AccumulatorHalfKaHmMerged::new(),
            dirty_piece,
    ...
    /// 指定インデックスのエントリを取得
    pub fn entry_at(&self, idx: usize) -> &AccumulatorEntryHalfKaHmMerged<L1> {
        &self.entries[idx]
    }

    /// 指定インデックスのエントリを取得（可変）
    pub fn entry_at_mut(&mut self, idx: usize) -> &mut AccumulatorEntryHalfKaHmMerged<L1> {
    ...
    pub fn get_prev_and_current_accumulators(
        &mut self,
        prev_idx: usize,
    ) -> (&AccumulatorHalfKaHmMerged<L1>, &mut AccumulatorHalfKaHmMerged<L1>) {
        let current_idx = self.current_idx;
...
impl<const L1: usize> Default for AccumulatorStackHalfKaHmMerged<L1> {
    fn default() -> Self {
        Self::new()
    }
}
...
// =============================================================================
// FeatureTransformerHalfKaHmMerged - const generics 版 Feature Transformer
// =============================================================================

/// HalfKaHmMerged Feature Transformer
pub struct FeatureTransformerHalfKaHmMerged<const L1: usize> {
    /// バイアス [L1]
    pub biases: Vec<i16>,
...
impl<const L1: usize> FeatureTransformerHalfKaHmMerged<L1> {
    ...
    pub(crate) fn share_weights(&mut self) {
        super::shared_weights::try_share(&mut self.weights, "FT weights (HalfKaHmMerged)");
    }
    ...
    /// Accumulatorをリフレッシュ
    pub fn refresh_accumulator(&self, pos: &Position, acc: &mut AccumulatorHalfKaHmMerged<L1>) {
    ...
    pub fn update_accumulator(
        ...
        acc: &mut AccumulatorHalfKaHmMerged<L1>,
        prev_acc: &AccumulatorHalfKaHmMerged<L1>,
    ) {
        for perspective in [Color::Black, Color::White] {
            let p = perspective as usize;
    ...
    pub fn forward_update_incremental(
        &self,
        pos: &Position,
        stack: &mut AccumulatorStackHalfKaHmMerged<L1>,
        source_idx: usize,
    ) -> bool {
    ...
    pub fn transform_raw(
        &self,
        acc: &AccumulatorHalfKaHmMerged<L1>,
        side_to_move: Color,
        output: &mut [i16],
    ) {
...
impl<const INPUT: usize, const OUTPUT: usize> AffineTransformHalfKaHmMerged<INPUT, OUTPUT> {
    /// パディング済み入力次元（32の倍数）
...
// =============================================================================
// NetworkHalfKaHmMerged - const generics 版統一ネットワーク
// =============================================================================

/// HalfKaHmMerged ネットワーク（const generics 版）
///
/// # 型パラメータ
/// - `L1`: FT出力次元（片側）
...
pub struct NetworkHalfKaHmMerged<
    const L1: usize,
    const FT_OUT: usize,
...
> {
    ...
    pub feature_transformer: FeatureTransformerHalfKaHmMerged<L1>,
    /// 隠れ層1: L1_INPUT → L2
    pub l1: AffineTransformHalfKaHmMerged<L1_INPUT, L2>,
    /// 隠れ層2: L2 → L3
    pub l2: AffineTransformHalfKaHmMerged<L2, L3>,
    /// 出力層: L3 → 1
    pub output: AffineTransformHalfKaHmMerged<L3, 1>,
    /// 評価値スケーリング係数
    pub fv_scale: i32,
    /// QA値（クリッピング閾値）
    pub qa: i16,
...
impl<
    ...
    const L3: usize,
    A: FtActivation,
> NetworkHalfKaHmMerged<L1, FT_OUT, L1_INPUT, L2, L3, A>
{
    /// コンパイル時制約
    ///
    ...
    pub fn read<R: Read + Seek>(reader: &mut R) -> io::Result<Self> {
        ...
        if arch_str.contains("Factorizer") {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "Unsupported model format: factorized (non-coalesced) HalfKaHmMerged^ model detected.\n\
        ...
        let mut feature_transformer = FeatureTransformerHalfKaHmMerged::read(reader)?;

        // FC layers ハッシュ
        reader.read_exact(&mut buf4)?;

        // l1: L1*2 → L2
        let l1 = AffineTransformHalfKaHmMerged::read(reader)?;

        // l2: L2 → L3
        let l2 = AffineTransformHalfKaHmMerged::read(reader)?;

        // output: L3 → 1
        let output = AffineTransformHalfKaHmMerged::read(reader)?;
    ...
    pub fn evaluate(&self, pos: &Position, acc: &AccumulatorHalfKaHmMerged<L1>) -> Value {
        ...
        for (i, &v) in l1_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L1 output[{i}] = {v} is out of expected range (NetworkHalfKaHmMerged<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        for (i, &v) in l2_out.0.iter().enumerate() {
            debug_assert!(
                v.abs() < 1_000_000,
                "L2 output[{i}] = {v} is out of expected range (NetworkHalfKaHmMerged<{}, {}, {}, {}>)",
                L1,
                L2,
                L3,
                A::name()
        ...
        debug_assert!(
            eval.abs() < 50_000,
            "Final evaluation {eval} is out of expected range (NetworkHalfKaHmMerged<{}, {}, {}, {}>). Raw output: {}",
            L1,
            L2,
            L3,
            A::name(),
    ...
    /// 新しい Accumulator を作成
    pub fn new_accumulator(&self) -> AccumulatorHalfKaHmMerged<L1> {
        AccumulatorHalfKaHmMerged::new()
    }

    /// 新しい AccumulatorStack を作成
    pub fn new_accumulator_stack(&self) -> AccumulatorStackHalfKaHmMerged<L1> {
        AccumulatorStackHalfKaHmMerged::new()
    }

    /// アーキテクチャ名を取得
    pub fn architecture_name(&self) -> String {
        format!("HalfKaHmMerged^{}x2-{}-{}-{}", L1, L2, L3, A::name())
    }
}
...
// L1=256, FT_OUT=512
/// HalfKaHmMerged 256x2-32-32 CReLU
pub type HalfKaHmMerged256CReLU = NetworkHalfKaHmMerged<256, 512, 512, 32, 32, CReLU>;
/// HalfKaHmMerged 256x2-32-32 SCReLU
pub type HalfKaHmMerged256SCReLU = NetworkHalfKaHmMerged<256, 512, 512, 32, 32, SCReLU>;
/// HalfKaHmMerged 256x2-32-32 PairwiseCReLU
pub type HalfKaHmMerged256Pairwise = NetworkHalfKaHmMerged<256, 512, 256, 32, 32, PairwiseCReLU>;

// L1=512, FT_OUT=1024, L2=8, L3=64
/// HalfKaHmMerged 512x2-8-64 CReLU
pub type HalfKaHmMerged512_8_64CReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 8, 64, CReLU>;
/// HalfKaHmMerged 512x2-8-64 SCReLU
pub type HalfKaHmMerged512_8_64SCReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 8, 64, SCReLU>;
/// HalfKaHmMerged 512x2-8-64 PairwiseCReLU
pub type HalfKaHmMerged512_8_64Pairwise =
    NetworkHalfKaHmMerged<512, 1024, 512, 8, 64, PairwiseCReLU>;

// L1=512, FT_OUT=1024, L2=8, L3=96
/// HalfKaHmMerged 512x2-8-96 CReLU
pub type HalfKaHmMerged512CReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 8, 96, CReLU>;
/// HalfKaHmMerged 512x2-8-96 SCReLU
pub type HalfKaHmMerged512SCReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 8, 96, SCReLU>;
/// HalfKaHmMerged 512x2-8-96 PairwiseCReLU
pub type HalfKaHmMerged512Pairwise = NetworkHalfKaHmMerged<512, 1024, 512, 8, 96, PairwiseCReLU>;

// L1=512, FT_OUT=1024, L2=32, L3=32
/// HalfKaHmMerged 512x2-32-32 CReLU
pub type HalfKaHmMerged512_32_32CReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 32, 32, CReLU>;
/// HalfKaHmMerged 512x2-32-32 SCReLU
pub type HalfKaHmMerged512_32_32SCReLU = NetworkHalfKaHmMerged<512, 1024, 1024, 32, 32, SCReLU>;
/// HalfKaHmMerged 512x2-32-32 PairwiseCReLU
pub type HalfKaHmMerged512_32_32Pairwise =
    NetworkHalfKaHmMerged<512, 1024, 512, 32, 32, PairwiseCReLU>;

// L1=1024, FT_OUT=2048, L2=8, L3=64
/// HalfKaHmMerged 1024x2-8-64 CReLU
pub type HalfKaHmMerged1024_8_64CReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 64, CReLU>;
/// HalfKaHmMerged 1024x2-8-64 SCReLU
pub type HalfKaHmMerged1024_8_64SCReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 64, SCReLU>;
/// HalfKaHmMerged 1024x2-8-64 PairwiseCReLU
pub type HalfKaHmMerged1024_8_64Pairwise =
    NetworkHalfKaHmMerged<1024, 2048, 1024, 8, 64, PairwiseCReLU>;

// L1=1024, FT_OUT=2048, L2=8, L3=96
/// HalfKaHmMerged 1024x2-8-96 CReLU
pub type HalfKaHmMerged1024CReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 96, CReLU>;
/// HalfKaHmMerged 1024x2-8-96 SCReLU
pub type HalfKaHmMerged1024SCReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 96, SCReLU>;
/// HalfKaHmMerged 1024x2-8-96 PairwiseCReLU
pub type HalfKaHmMerged1024Pairwise = NetworkHalfKaHmMerged<1024, 2048, 1024, 8, 96, PairwiseCReLU>;

// L1=1024, FT_OUT=2048, L2=8, L3=32
/// HalfKaHmMerged 1024x2-8-32 CReLU
pub type HalfKaHmMerged1024_8_32CReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 32, CReLU>;
/// HalfKaHmMerged 1024x2-8-32 SCReLU
pub type HalfKaHmMerged1024_8_32SCReLU = NetworkHalfKaHmMerged<1024, 2048, 2048, 8, 32, SCReLU>;
/// HalfKaHmMerged 1024x2-8-32 PairwiseCReLU
pub type HalfKaHmMerged1024_8_32Pairwise =
    NetworkHalfKaHmMerged<1024, 2048, 1024, 8, 32, PairwiseCReLU>;

// L1=768, FT_OUT=1536, L2=16, L3=64
/// HalfKaHmMerged 768x2-16-64 CReLU
pub type HalfKaHmMerged768CReLU = NetworkHalfKaHmMerged<768, 1536, 1536, 16, 64, CReLU>;
/// HalfKaHmMerged 768x2-16-64 SCReLU
pub type HalfKaHmMerged768SCReLU = NetworkHalfKaHmMerged<768, 1536, 1536, 16, 64, SCReLU>;
/// HalfKaHmMerged 768x2-16-64 PairwiseCReLU
pub type HalfKaHmMerged768Pairwise = NetworkHalfKaHmMerged<768, 1536, 768, 16, 64, PairwiseCReLU>;
...
mod tests {
    ...
    fn test_padded_input() {
        ...
        assert_eq!(AffineTransformHalfKaHmMerged::<33, 96>::PADDED_INPUT, 64);
        assert_eq!(AffineTransformHalfKaHmMerged::<96, 1>::PADDED_INPUT, 96);
        assert_eq!(AffineTransformHalfKaHmMerged::<1024, 8>::PADDED_INPUT, 1024);
        assert_eq!(AffineTransformHalfKaHmMerged::<2048, 8>::PADDED_INPUT, 2048);
    }
    ...
    fn test_parse_qa_from_arch() {
        ...
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKaHmMerged[73305->512x2],fv_scale=5,l1_input=1024,l2=8,l3=96,qa=255,qb=64"
            ),
            Some(255)
        );
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKaHmMerged[73305->512x2],fv_scale=5,l1_input=1024,l2=8,l3=96,qa=127,qb=64"
            ),
            Some(127)
        );
        ...
        assert_eq!(
            parse_qa_from_arch(
                "Features=HalfKaHmMerged[73305->512x2],Network=AffineTransform[1<-96]"
            ),
            None
        );
    }
