...
impl PerfWrapper {
    ...
    fn run_search(
        ...
        let bestmove = bestmove.ok_or_else(|| {
            anyhow!("{}: timed out waiting for bestmove for position {}", self.label, position.name)
        })?;
    ...
    fn new() -> Result<Self> {
        ...
        unsafe {
            if libc::pipe(ctl.as_mut_ptr()) == -1 {
                return Err(std::io::Error::last_os_error()).context("pipe() failed for control");
            }
...
fn main() -> Result<()> {
    ...
    for handle in handles {
        let mut shard_samples = handle.join().map_err(|_| anyhow!("shard thread panicked"))??;
...
fn build_summary(samples: &[RunSample]) -> Result<ComparisonSummary> {
    ...
    Ok(ComparisonSummary {
        ...
        cycles_per_node_delta_pct: pct_delta(candidate.cycles_per_node, baseline.cycles_per_node),
