...
`kept` はフィルタ（汚染除去・重複除去）を通過した総レコード数（reservoir サンプリング前）、
`written` は実際に出力したレコード数です。`--target N`（N>0）指定時は reservoir で N 件に
絞るため `kept`（通過総数）≥ `written`（=N、ただし通過総数が N 未満なら通過総数）になります。
