...
- 中央: 盤面（内容ぴったりの固定幅・罫線付き。先手の駒は黄、後手の駒は シアン で
  ...
  表示し、最終手の移動先（駒）と移動元（空マス）を背景色（緑）＋太字でハイライトする。手番・持駒・
  王手表示も指了後の局面を反映する（「王手」＝最終手が王手だったこと。ただし pass 手だけは
  指了前の局面のまま表示する））
- 右: 指し手一覧（棋譜風ラベル＋各手の注釈を淡色で並べる：評価値・depth・seldepth・
  nodes・nps・経過（engine 名は対局ラベルと重複、think_limit_ms は depth 制限の対局で
  既定値が出て誤解を招くため、いずれも省略）。パネル幅に収まらなければ末尾は切れる。現在の手をハイライト。手数は絶対手数で表示する（JSONL は対局内
