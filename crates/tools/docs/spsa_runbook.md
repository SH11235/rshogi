...
| `--concurrency 384 --seeds 1,2 --games-per-iteration 64` | `384 / 2 = 192` だが `min(192, 64) = 64` で clamp → 256 worker しか動かず 128 遊休 | seed を増やす (`--seeds 1,2,3,4,5,6`) か `--concurrency 128` に減らす |
| `--concurrency 10 --seeds 1,2,3,4` | `10 / 4 = 2` worker/seed → 合計 8、2 worker 遊休 | `--concurrency 8` (倍数化) |
