...
- 本 runbook は **v4 仕様** (fishtest 整合の paired antithetic / stochastic
  rounding / 1 batch = 1 update / 単一 seed) を前提とする。v3 までの
  multi-seed 機能 (`--seeds` / `--parallel-seeds`) は撤去済み。
  CLI / format / CSV の破壊的変更点は `CHANGELOG.md` の v4 エントリを参照。
...
| フラグ | 意味 |
|---|---|
| `--total-pairs N` | SPSA 全体の game pair 上限 (= fishtest `num_iter`)。total_games = 2N |
| `--batch-pairs B` | 1 batch あたりの game pair 数 (= fishtest worker `game_pairs`、既定 8)。1 batch で `2B` 局を消化し θ を 1 回更新、k は `+= B` |
| `--seed S` | 単一 base_seed (省略時はランダム)。SPSA の RNG stream は seed と batch index から決定論的に生成 |

`<run-dir>` には以下が自動生成される:

| ファイル | 内容 |
|---|---|
| `state.params` | SPSA の live 状態 (batch ごとに上書き、is_int でも `{:.6}` 固定桁) |
| `final.params` | SPSA 正常完了時の確定スナップショット (`tune.py apply` 用) |
| `meta.json` | resume 用メタデータ (format_version=4) |
| `values.csv` | 各 batch のパラメータ値履歴 |
| `stats.csv` | 1 batch = 1 行の統計 (v3 までの per-seed 統計は撤去) |
...
v4 の paired antithetic では、**1 pair (2 局)** ごとに 1 つの開始局面を抽選し、
**先後を入れ替えて 2 局連続** で消化する (fishtest worker と同じ流儀)。これにより
...
| 設定 | 目安 | 効くもの |
|---|---|---|
| `--total-pairs N` | 数千〜数万 (LMR/枝刈り系の小規模グループなら 5,000〜10,000、全体は 30,000+) | 収束確度。SPSA schedule の k 軸上限であり、これが `n_iter` 相当 |
...
`run_batch_games_parallel` は `min(concurrency, 2 × batch_pairs)` 個の worker を
起動する。`--concurrency` を `2 × batch_pairs` より大きくしても余剰 worker は
仕事がなく無駄になるので、**`--concurrency ≤ 2 × batch_pairs`** に揃えるのが
基本。多コアを活かしたい場合は `--batch-pairs` を増やすか、`--seed` を
変えた **独立 run dir** を並列実行する (v3 の `--parallel-seeds` 相当の代替)。
...
集計する (v3 の `stats_aggregate.csv` 自動生成は撤去された)。
...
resume 時の不変条件チェック (実装側で hard bail):

- `completed_pairs <= total_pairs`
- 中間 batch では `completed_pairs == completed_iterations × batch_pairs`
- v4 meta の `total_pairs` / `batch_pairs` が CLI 指定値と一致
...
| `<run-dir>/final.params` | SPSA 正常完了時の確定スナップショット (`tune.py apply` 用) | (なし) |
| `<run-dir>/meta.json` | resume 用メタデータ (format_version=4) | `--meta-file` |
| `<run-dir>/values.csv` | 各 batch のパラメータ値履歴 | `--param-values-csv` |
| `<run-dir>/stats.csv` | 各 batch の統計 (1 batch = 1 行) | `--stats-csv` |
...
> `--use-existing-state-as-init` を**明示**指定すること (silent fresh は事故の
> 温床だったため)。
...
に `0, <初期値>, ...` 行が記録される (v3 までの呼称は「iter 0 snapshot」)。
resume 時は append のため重複しない。これにより「初期値が何であったか」を
CSV だけで完全に追える。
...
を pandas/awk で concat してから集計する。v4 では `stats_aggregate.csv` の
自動生成は撤去された。
...
- 複数 seed の探索: `--seed` を変えた **独立 run dir** で並列実行 (v3 の `--parallel-seeds` 撤去)
...
| 再開メタデータ | `<run-dir>/meta.json` (format_version=4) | `--meta-file` | — |
...
v4 では 1 batch = 1 行で出力される。CSV header:
...
| 列 | 意味 |
|---|---|
| `iteration` | 1-indexed の batch 番号 (v3 でも同名フィールドだが意味は「batch 番号」に再解釈) |
...
| `total_games` | これまでの累積対局数 (= 2 × completed_pairs) |

v3 までの `seed` カラムは撤去された (multi-seed 機能の廃止と連動)。`games` 列は
`batch_pairs` に rename + 値の意味も変わった (game 数 → game pair 数)。

### 9.2 `values.csv` (パラメータ履歴、wide 形式)

各列が 1 パラメータ、各行が 1 batch。`spsa_stats_to_plot_csv` で window 平均を
かけてプロットすると個別パラメータの収束軌跡が見やすい。is_int パラメータも
`{:.6}` 固定桁で書き出される (v4 では θ 内部状態を f64 のまま保持し、engine
送信時のみ stochastic round するため)。
...
v4 では `result_variance_threshold` の比較対象が **`|raw_result| / batch_pairs`**
(0..1 の正規化値) に変更されている。v3 までは「seed 横断分散」を比較していた
ため、v3 の閾値値をそのまま流用すると過敏 / 鈍感になる。閾値値は v4 仕様で
再調整すること (枝刈り系は感度高、history 初期値は低、等)。
...
- **できない**: 旧 run の `completed_iterations` / `total_games` / SPSA schedule
  ...
  復元できないため resume は不可
- v3 までの multi-seed run (`--seeds 2,3,...`) や奇数 `games_per_iteration` の run
  は、format 上自動検出できないため **silent migration では完全引き継ぎできない**。
  最終値だけを新 run の canonical として再投入するのが安全 (本節の手順)
...
# (`final.params` が無い場合は ${OLD_RUN}/state.params を使う。v3 以前で命名が
#  `tuned.params` だった旧 run は当該ファイルを使う)
...
- v3 → v4: warning を出して **silent migrate** される (`completed_iterations`
  をそのまま batch 番号として継承し、`completed_pairs` を
  `completed_iterations × batch_pairs` で再構築)。CLI 側で `--total-pairs` /
  `--batch-pairs` の再指定が必要。**multi-seed run / 奇数 `games_per_iteration`
  の v3 run** は schema 上自動検出できないため、silent migration で完全継承
  できない。最終値だけ §10.7 の手順で新 run の canonical として再投入する
  のが安全
- v2 以前: hard bail。新規 run dir で `--init-from <canonical>` から fresh start

### `--seeds は v4 で撤去されました` / `--parallel-seeds は v4 で撤去されました`

v4 では multi-seed 機能が完全に廃止された。代替は **`--seed` を変えた独立
run dir を並列実行** する運用。詳細は §3.3 の `batch_pairs と concurrency
の関係` セクション参照。
...
- 単一 seed のばらつきを確認したい場合は、`--seed` を変えた独立 run dir を
  数本 (3〜5 本) 走らせて `stats.csv` を比較する (v3 の multi-seed 機能の代替)
