...
- A/A が示すのは計測の再現性で、ビルドごとのコード配置差は含まない。baseline と candidate が
  別ビルドなら、同じペアを何度測っても配置差は同じ向きに出る。instructions/node が横ばいで
  cycles/node だけが 1 % 前後動く結果は、同一 binary 内で経路を切り替える計測か、配置を変えた
  複数ビルドでの符号の一致で確かめる。
