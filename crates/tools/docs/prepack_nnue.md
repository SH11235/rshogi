...
次のロードから有効になる。ロードが走るのはエンジン再起動と `EvalFile` 再設定に加えて、
`SPSA_NET_*` の係数を変えたとき（`reload_net_deltas_if_dirty` が `EvalFile` のパスを
開き直す）。つまり稼働中に**別のモデル**で差し替えると、その後の係数変更を契機に
セッションの途中でモデルが入れ替わる。差し替えたくないなら別パスへ置く。
