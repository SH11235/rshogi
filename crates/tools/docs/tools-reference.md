...
| ツール | 説明 |
|--------|------|
| `spsa` | SPSA チューナー（対局履歴をエンジンに送信し、千日手を自動終局）。paired antithetic + stochastic rounding + 1 batch = 1 update のスケジュールで対局を回す。複数 seed の探索は `--seed` を変えた独立 run dir を別プロセスで並列実行する |
