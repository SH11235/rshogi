...
| ツール | 説明 |
|--------|------|
| `tournament` | 中断時も回収結果を保存し run 状態を明示。複数エンジンの round-robin 並列トーナメント、error ペア再対局、SPRT 検定。`--seed` による matchup ごとの決定的な開始局面選択と seed 付き meta 出力、千日手の自動終局・エンジンへの対局履歴送信に対応（[詳細](docs/tournament.md)） |
